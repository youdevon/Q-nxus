...
export async function createPerformanceAppraisal(
  ...
  const supervisorUserId = nullableText(
    formData,
    "supervisorUserId",
  )
  const title = textValue(formData, "title")
  const appraisalNumber = nullableText(
    formData,
    "appraisalNumber",
  )
  const periodStart = parseDate(
    textValue(formData, "periodStart"),
  )
  const periodEnd = parseDate(
    textValue(formData, "periodEnd"),
  )
  const reviewDueDate = parseDate(
    textValue(formData, "reviewDueDate"),
  )
  const ratingScaleValue = textValue(
    formData,
    "ratingScale",
  )
  ...
  if (!assignmentId) {
    fieldErrors.assignmentId =
      "Select the assignment being appraised."
  }
  ...
  if (!periodStart) {
    fieldErrors.periodStart =
      "Enter the appraisal period start date."
  }

  if (!periodEnd) {
    fieldErrors.periodEnd =
      "Enter the appraisal period end date."
  }
  ...
  if (
    ...
    fieldErrors.ratingScale =
      "Select a valid rating scale."
  }
  ...
  if (!assignment) {
    return {
      status: "error",
      message:
        "The selected employee assignment is invalid.",
    }
  }
  ...
  if (!assignment.jobDescription) {
    return {
      status: "error",
      message:
        "The selected assignment does not have a job description.",
    }
  }
  ...
  if (assignment.jobDescription.criteria.length === 0) {
    return {
      status: "error",
      message:
        "The selected job description has no active criteria.",
    }
  }
  ...
  if (periodStart! < assignment.startDate) {
    return {
      status: "error",
      message:
        "The appraisal period cannot begin before the assignment.",
    }
  }
  ...
  if (duplicate) {
    return {
      status: "error",
      message:
        "An appraisal already exists for this employee and period.",
    }
  }
  ...
  try {
    const appraisal = await prisma.$transaction(
      async (transaction) => {
        const created =
          await transaction.performanceAppraisal.create({
            data: {
              employeeId,
              assignmentId: assignment.id,
              jobDescriptionId:
                assignment.jobDescription!.id,
              supervisorUserId,
              appraisalNumber,
              title,
              periodStart: validatedPeriodStart,
              ...
              ratingScale:
                ratingScaleValue as PerformanceRatingScale,
              maximumScore: new Prisma.Decimal(
                maximumScore,
              ),
              criteria: {
                create:
                  assignment.jobDescription!.criteria.map(
                    (criterion) => ({
    ...
    redirect(
      `/people/employees/${employeeId}/appraisals/${appraisal.id}`,
    )
  } catch (error: unknown) {
    if (
    ...
    console.error(
      "Unable to create performance appraisal:",
      error,
    )
    ...
    return {
      status: "error",
      message:
        "The performance appraisal could not be created.",
    }
  }
}
