...
export async function closeEmploymentContract(
  ...
  const closureStatusValue = textValue(formData, "closureStatus");
  const effectiveDate = parseDate(textValue(formData, "effectiveDate"));
  const terminationReason = nullableText(formData, "terminationReason");
  const documentReference = nullableText(formData, "documentReference");
  const updateEmployeeStatus = formData.get("updateEmployeeStatus") === "on";
  const employeeStatusValue = textValue(formData, "employeeStatus");
  const confirmed = formData.get("confirmed") === "on";

  const permittedStatuses = new Set<EmploymentContractStatus>([
  ...
  if (!permittedStatuses.has(closureStatusValue as EmploymentContractStatus)) {
    fieldErrors.closureStatus = "Select a valid closure type.";
  }
  ...
  if (closureStatusValue === "TERMINATED" && !terminationReason) {
    fieldErrors.terminationReason =
      "Enter the reason for terminating the contract.";
  }

  if (!confirmed) {
    fieldErrors.confirmed = "Confirm that the contract should be closed.";
  }
  ...
  if (
    ...
    fieldErrors.employeeStatus = "Select a valid employee status.";
  }
  ...
  if (
    ...
    return {
      status: "conflict",
      message: "The contract changed elsewhere. Refresh before continuing.",
    };
  }
  ...
  if (effectiveDate! < contract.startDate) {
    return {
      status: "error",
      message: "The closure date cannot be before the contract start date.",
    };
  }
  ...
  try {
    await prisma.$transaction(async (transaction) => {
      const result = await transaction.employmentContract.updateMany({
        where: {
          id: contractId,
          employeeId,
          isCurrent: true,
        ...
        data: {
          status: closureStatusValue as EmploymentContractStatus,
          isCurrent: false,
          endDate:
          ...
          terminationDate:
            closureStatusValue === "TERMINATED" ? effectiveDate : null,
          terminationReason:
            closureStatusValue === "TERMINATED" ? terminationReason : null,
          documentReference: documentReference ?? contract.documentReference,
        },
      });
      ...
      if (updateEmployeeStatus) {
        await transaction.employee.update({
          ...
          data: {
            employmentStatus: employeeStatusValue as EmploymentStatus,
            terminationDate:
              employeeStatusValue === EmploymentStatus.TERMINATED ||
              employeeStatusValue === EmploymentStatus.RETIRED
                ? effectiveDate
                : undefined,
          },
        });
    ...
    redirect(`/people/employees/${employeeId}/contracts/${contractId}`);
  } catch (error: unknown) {
    if (error instanceof Error && error.message === "NEXT_REDIRECT") {
      throw error;
    }

    if (error instanceof Error && error.message === "CONTRACT_CONFLICT") {
      return {
        status: "conflict",
        message:
