...
export async function createSystemNotification(
  ...
  const notification = await prisma.notification.create({
    data: {
      ...
      severity:
        input.severity ?? NotificationSeverity.INFORMATION,
  ...
  if (input.email) {
    ...
    for (const recipient of uniqueRecipients) {
      ...
      await queueEmail({
        ...
        textBody:
          input.email.textBody ?? input.message,
        htmlBody: wrapSystemEmailHtml({
          heading: input.title,
          bodyHtml,
          actionLabel:
            input.email.actionLabel ??
            (actionUrl ? "Open in Q-NXUS" : null),
          actionUrl,
        }),
        priority:
          input.email.priority ??
