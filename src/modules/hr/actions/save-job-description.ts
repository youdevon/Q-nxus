...
function parseCriteria(formData: FormData) {
  ...
  for (const rowId of rowIds) {
    ...
    const criterionTypeValue = textValue(formData, `criterionType:${rowId}`);
    ...
    criteria.push({
      criterionType: criterionTypeValue as JobCriterionType,
      title,
      description: nullableText(formData, `criterionDescription:${rowId}`),
      measurement: nullableText(formData, `criterionMeasurement:${rowId}`),
      weight: new Prisma.Decimal(weightValue),
      sortOrder: Number.isInteger(sortOrderValue) ? sortOrderValue : 0,
      isActive: formData.get(`criterionActive:${rowId}`) === "on",
    });
  }
  ...
  const totalWeight = criteria
    .filter((criterion) => weightedTypes.has(criterion.criterionType))
    .reduce(
...
export async function createJobDescription(
  ...
  if (!positionId || title.length < 2 || !effectiveFrom) {
    return {
      status: "error",
      message: "Position, title and effective-from date are required.",
    };
  }
  ...
  try {
    ...
    const created = await prisma.$transaction(async (transaction) => {
      ...
      const latest = await transaction.positionJobDescription.findFirst({
        where: {
          positionId,
        },
        orderBy: {
      ...
      const jobDescription = await transaction.positionJobDescription.create({
        data: {
          positionId,
          versionNumber,
          title,
          summary: nullableText(formData, "summary"),
          positionPurpose: nullableText(formData, "positionPurpose"),
          reportsTo: nullableText(formData, "reportsTo"),
          supervisoryResponsibility: nullableText(
            formData,
            "supervisoryResponsibility",
          ),
          qualifications: nullableText(formData, "qualifications"),
          requiredExperience: nullableText(formData, "requiredExperience"),
    ...
    const message =
      error instanceof Error && error.message === "TOTAL_WEIGHT_EXCEEDS_100"
        ? "The total appraisal weighting cannot exceed 100%."
        : error instanceof Error && error.message === "INVALID_CRITERION_WEIGHT"
...
export async function updateJobDescription(
  ...
  try {
    ...
    const current = await prisma.positionJobDescription.findUnique({
      where: {
        id,
      },
      include: {
    ...
    const saved = await prisma.$transaction(async (transaction) => {
      const updatedCount = await transaction.positionJobDescription.updateMany({
        where: {
          id,
      ...
      await transaction.auditEvent.create({
        data: {
          ...
          oldValues: {
            ...
            supervisoryResponsibility: current.supervisoryResponsibility,
          ...
          newValues: {
            ...
            supervisoryResponsibility: updated.supervisoryResponsibility,
...
export async function activateJobDescription(
  ...
  await prisma.$transaction(async (transaction) => {
    const current = await transaction.positionJobDescription.findUnique({
      where: {
        id,
      },
    });
...
export async function cloneJobDescription(formData: FormData): Promise<void> {
  const actor = await requireActor("people.manage");
  if (!actor.ok) {
    throw new Error(actor.message);
  }
  ...
  const cloned = await prisma.$transaction(async (transaction) => {
    const source = await transaction.positionJobDescription.findFirst({
      where: {
        id,
        positionId,
      },
    ...
    const created = await transaction.positionJobDescription.create({
      data: {
        positionId,
        versionNumber,
        title: source.title,
        ...
        supervisoryResponsibility: source.supervisoryResponsibility,
  ...
  revalidatePath(`/people/structure/positions/${positionId}/job-descriptions`);
...
export async function retireJobDescription(formData: FormData): Promise<void> {
  const actor = await requireActor("people.manage");
  if (!actor.ok) {
    throw new Error(actor.message);
  }
  ...
  const metadata = await getAuditRequestMetadata(formData);
  await prisma.$transaction(async (transaction) => {
    const current = await transaction.positionJobDescription.findFirst({
      where: {
        id,
        positionId,
      },
    ...
    const updated = await transaction.positionJobDescription.update({
      where: {
        id,
      },
      data: {
