...
function validateCategory(formData: FormData) {
  ...
  if (code && code.length > 20) {
    fieldErrors.code =
      "Allowance code cannot exceed 20 characters."
  }
  ...
  return {
    ...
    values: {
      ...
      description,
      isTaxableDefault:
        formData.get("isTaxableDefault") === "on",
...
export async function createAllowanceCategory(
  ...
  if (duplicate) {
    return {
      status: "error",
      message:
        "An allowance category with this name already exists.",
    }
  }
  ...
  const metadata = await requestMetadata()
    ...
    const category = await prisma.$transaction(
      async (transaction) => {
        const created =
          await transaction.allowanceCategory.create({
            data: {
              organizationId: organization.id,
              ...validation.values,
            },
        ...
        await transaction.auditEvent.create({
          data: {
            ...
            newValues: {
              ...
              isTaxableDefault:
                created.isTaxableDefault,
              includedInGratuityDefault:
                created.includedInGratuityDefault,
    ...
    console.error(
      "Unable to create allowance category:",
      error,
    )
    ...
    return {
      status: "error",
      message:
        "The allowance category could not be created.",
    }
  }
}
...
export async function updateAllowanceCategory(
  ...
  if (!id || !submittedUpdatedAt) {
    return {
      status: "error",
      message:
        "The allowance category record is incomplete.",
    }
  }
  ...
  if (!current) {
    return {
      status: "error",
      message:
        "The allowance category no longer exists.",
    }
  }
  ...
  const metadata = await requestMetadata()
    ...
    const result = await prisma.$transaction(
      async (transaction) => {
        const updateResult =
          await transaction.allowanceCategory.updateMany({
            where: {
              id,
        ...
        const updated =
          await transaction.allowanceCategory.findUniqueOrThrow({
            where: {
              id,
            },
          })
        ...
        await transaction.auditEvent.create({
          data: {
            ...
            oldValues: {
              ...
              isTaxableDefault:
                current.isTaxableDefault,
              includedInGratuityDefault:
                current.includedInGratuityDefault,
            ...
            newValues: {
              ...
              isTaxableDefault:
                updated.isTaxableDefault,
              includedInGratuityDefault:
                updated.includedInGratuityDefault,
    ...
    if (!result) {
      return {
        status: "conflict",
        message:
          "The allowance category changed while being saved.",
      }
    }

    revalidatePath("/administration/allowances")
    revalidatePath(
      `/administration/allowances/${id}`,
    )
    revalidatePath(
      `/administration/allowances/${id}/edit`,
    )
    ...
    console.error(
      "Unable to update allowance category:",
      error,
    )
    ...
    return {
      status: "error",
      message:
        "The allowance category could not be updated.",
    }
  }
}
