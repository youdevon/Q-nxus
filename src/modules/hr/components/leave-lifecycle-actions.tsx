...
export function LeaveLifecycleActions({
  ...
  const [withdrawState, withdrawAction, withdrawPending] =
    useActionState(withdrawLeaveRequest, initialState)
  const [cancelState, cancelAction, cancelPending] =
    useActionState(cancelLeaveRequest, initialState)

  const state =
    withdrawState.status !== "idle" ? withdrawState : cancelState
