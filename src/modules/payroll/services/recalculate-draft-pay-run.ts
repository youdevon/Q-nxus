...
import { isPayRunEditable, isPayRunMutable } from "@/src/modules/payroll/lib/pay-run-lifecycle";
...
export async function rebuildDraftPayslipFromLineItems(input: {
  ...
  if (!isPayRunEditable(input.payRun.status)) {
    return {
      ok: false,
      message:
        input.payRun.status === "APPROVED"
