...
export async function getEmailAdministrationData() {
  ...
  const configurationErrors = validateSmtpConfiguration(configuration);

  const [pending, processing, sent, failed, cancelled, recent] =
    await Promise.all([
  ...
  return {
    smtp: {
      ...
      usernameConfigured: configuration.username.length > 0,
      passwordConfigured: configuration.password.length > 0,
