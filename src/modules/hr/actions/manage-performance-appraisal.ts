...
function revalidateAppraisal(
  employeeId: string,
  appraisalId: string,
) {
  revalidatePath(
    `/people/employees/${employeeId}/appraisals`,
  )

  revalidatePath(
    `/people/employees/${employeeId}/appraisals/${appraisalId}`,
  )
...
export async function savePerformanceAppraisalRatings(
  ...
  const employeeComments = nullableText(
    formData,
    "employeeComments",
  )

  const supervisorComments = nullableText(
    formData,
    "supervisorComments",
  )

  const developmentPlan = nullableText(
    formData,
    "developmentPlan",
  )
  ...
  if (!submittedCriteria) {
    return {
      status: "error",
      message:
        "The appraisal rating information could not be read.",
    }
  }

  const appraisal =
    await prisma.performanceAppraisal.findFirst({
      where: {
        id: appraisalId,
        employeeId,
      },
  ...
  if (
    ...
    return {
      status: "conflict",
      message:
        "The appraisal changed elsewhere. Refresh before saving.",
    }
  }

  const maximumScore = maximumForScale(
    appraisal.ratingScale,
  )
  ...
  const normalizedCriteria = submittedCriteria.map(
    (submitted, index) => {
      const criterion = criterionMap.get(submitted.id)

      if (!criterion) {
        fieldErrors[`criterion.${index}`] =
          `Criterion ${index + 1} is invalid.`

        return null
      }

      const employeeRating = parseOptionalRating(
        submitted.employeeRating,
      )

      const supervisorRating = parseOptionalRating(
        submitted.supervisorRating,
      )

      const finalRating = parseOptionalRating(
        submitted.finalRating,
      )
      ...
      return {
        ...
        weightedScore,
        employeeComments:
          submitted.employeeComments.trim() || null,
        supervisorComments:
          submitted.supervisorComments.trim() || null,
  ...
  const validCriteria = normalizedCriteria.filter(
    (
      criterion,
    ): criterion is NonNullable<typeof criterion> =>
      criterion !== null,
  )

  const overallScore = validCriteria.reduce(
    (total, criterion) =>
      total + (criterion.weightedScore ?? 0),
    0,
  )
  ...
  try {
    const saved = await prisma.$transaction(
      async (transaction) => {
        const updateResult =
          await transaction.performanceAppraisal.updateMany({
            where: {
              id: appraisalId,
              employeeId,
    ...
    console.error(
      "Unable to save performance appraisal ratings:",
      error,
    )
    ...
    return {
      status: "error",
      message:
        "The performance appraisal ratings could not be saved.",
    }
  }
}
...
async function transitionAppraisal(
  ...
  if (
    !appraisal ||
    !options.allowedStatuses.includes(appraisal.status)
  ) {
    return
  }
...
export async function acknowledgePerformanceAppraisal(
  ...
  await transitionAppraisal(formData, {
    ...
    nextStatus:
      PerformanceAppraisalStatus.EMPLOYEE_ACKNOWLEDGED,
    action: "ACKNOWLEDGE",
